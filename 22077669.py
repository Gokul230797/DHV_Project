...
def process_world_bank_data(population_data):
    ...
    world_bank_data.drop(columns=['Country Code', 'Series Code', '2001 [YR2001]', '2002 [YR2002]', '2003 [YR2003]', '2004 [YR2004]', '2005 [YR2005]', '2006 [YR2006]', '2007 [YR2007]',
                                  '2008 [YR2008]', '2009 [YR2009]', '2010 [YR2010]', '2011 [YR2011]', '2012 [YR2012]'], inplace=True)
...
def barplot_urban_growth_data(ax, world_bank_data):
    ...
    sns.barplot(ax=ax, x='Country', y='Urban Population Growth (Annual %)', hue='Year', data=urban_growth_data_melted,
...
def lineplot_urban_growth_data(ax, world_bank_data):
    ...
    sns.lineplot(ax=ax, x='Year', y='Urban Population Growth (Annual %)', hue='Country', data=urban_growth_data_melted,
                 marker='o', markersize=8, linewidth=2)

    ax.set_title('Urban Population Growth (Annual %) Over the Years', fontsize=16, weight='bold')
    ax.set_xlabel('Year', fontsize=14)
    ax.set_xticks(range(2015, 2020, 1))
    ax.set_ylabel('Urban Population Growth (Annual %)', fontsize=14)

    ax.legend(title='Country', bbox_to_anchor=(1.05, 1), loc='best', fontsize=10)

def pie_chart_largest_city_population(ax, world_bank_data):
    """
    Plot a pie chart for population distribution in the largest city (% of urban population) in 2019.
    ...
    ax.axis('equal')
    ax.legend(title='Country', labels=top_countries['Country'], loc='upper left', bbox_to_anchor=(1, 0.8), fontsize=10)
...
def histogram_rural_population(ax, world_bank_data):
    """
    Plot a histogram for the distribution of rural population across countries for the years 2015 to 2019.
    ...
    sns.histplot(ax=ax, data=rural_population_data_melted, x='Rural Population', bins=20, color='skyblue')
    ax.set_title('Distribution of Rural Population Across Countries (2015-2019)', fontsize=16, weight='bold')
...
fig, axs = plt.subplots(2, 2, figsize=(20, 12), gridspec_kw={'hspace': 0.6, 'wspace': 0.4})
...
fig.suptitle('Population Growth Analysis - Urban & Rural (2015 - 2019)', fontsize=40, weight='bold',x=1, y=1)
...
plt.figtext(0.92, 0.5, combined_description, ha='left', va='center', fontsize=18, bbox=dict(facecolor='white', alpha=0.8, boxstyle='round,pad=0.5'),weight='bold')
